...
#pragma mark - get
- (MyMXAPPViewRequest *)afn_sharedManager {
    MyMXAPPViewRequest *request = [MyMXAPPViewRequest sharedManager];
...
- (NSDictionary *)createParams:(NSDictionary *)param
                 viewParameter:(MyMXAPPViewRequest *)viewParameter {
...
- (void)useMemoryCache:(NSString *)url complete:(AFNManagerCompleteBlock)complete {
    if (complete) {
        ...
        MXAPPResult *result = [[MXAPPResult alloc] init];
...
- (void)createRequest:(NSString *)url
             ...
             complete:(AFNManagerCompleteBlock)complete {
    MyMXAPPViewRequest *viewParameter = [self afn_request];
...
//            }
    ...
    [AFNNetworkManager POST:url params:param progress:^(CGFloat currentProgress, CGFloat maxProgress, CGFloat percentage) {
        // 过程
        if (progress) {
            progress(currentProgress, maxProgress, percentage);
        }
    } complete:^(MXAPPResult * _Nonnull result) {
        // 停止刷新
        [weak endRefresh];
        // 成功
