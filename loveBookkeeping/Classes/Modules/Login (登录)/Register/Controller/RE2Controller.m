...
- (void)getCodeRequest {
    ...
    @weakify(self)
    [AFNNetworkManager POST:CreateCoderequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
...
- (void)validateRequest {
    ...
    [self showProgressHUD];
    @weakify(self)
    [AFNNetworkManager POST:ValidateCoderequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
        if (result.status == ServiceCodeSuccess) {
