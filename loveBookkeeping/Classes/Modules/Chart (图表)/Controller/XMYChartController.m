...
#pragma mark - 实现
@implementation XMYChartController
...
- (void)chartTableClick:(NSIndexPath *)indexPath {
    ...
    if (!_cmodel) {
        XMYChartController *vc = [[XMYChartController alloc] init];
        vc.cmodel = model;
    ...
    if (!_seg) {
        ...
        [[_seg.seg rac_signalForControlEvents:UIControlEventValueChanged] subscribeNext:^(UISegmentedControl *seg) {
            ...
            [self setDate:({
                ...
                XMYChartSubModel *model = self.subdate.sModels[index][indexPath.row];
...
- (ChartDate *)subdate {
    if (!_subdate) {
        ...
        [_subdate setComplete:^(XMYChartSubModel *model) {
            @strongify(self)
            NSInteger month = model.month == -1 ? 1 : model.month;
            NSInteger day = model.day == -1 ? 1 : model.day;
