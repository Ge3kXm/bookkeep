...
#pragma mark - 声明
@interface MXLoveBKCController()<UIScrollViewDelegate>
...
#pragma mark - 实现
@implementation MXLoveBKCController
...
- (void)bendiData {
    ...
    model1.list = ({
        ...
        while(sysHasPayArr.count > 3) {
            [sysHasPayArr removeLastObject];
        }
...
- (void)getCategoryListRequest {
    @weakify(self)
    [self.scroll createRequest:CategoryListRequest params:@{} complete:^(MXAPPResult *result) {
        @strongify(self)
