...
- (void)changeIconRequest:(UIImage *)image {
    ...
    [AFNNetworkManager POST:ChangeIconRequest params:param andImages:@[image] progress:nil complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
...
- (void)changeNickRequest:(NSString *)nickName {
    ...
    [self.afn_request setAfn_useCache:false];
    [self showProgressHUD:@"修改中"];
    [AFNNetworkManager POST:NicknameRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
        if (result.status == ServiceCodeSuccess) {
            // 更新数据
...
- (void)bindThirdRequest {
    ...
    [self umQQUserInfo:^(KKSocialUserInfoResponse *resp) {
        ...
        [AFNNetworkManager POST:BindThirdRequest params:param andImages:@[resp.icon] progress:nil complete:^(MXAPPResult *result) {
            [self hideHUD];
            if (result.status == ServiceCodeSuccess) {
                UserModel *model = [UserInfo loadUserInfo];
...
- (void)changeSexRequest:(NSInteger)sex {
    ...
    [self.afn_request setAfn_useCache:false];
    [self showProgressHUD:@"修改中"];
    [AFNNetworkManager POST:ChangeSexRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
        if (result.status == ServiceCodeSuccess) {
            // 更新数据
