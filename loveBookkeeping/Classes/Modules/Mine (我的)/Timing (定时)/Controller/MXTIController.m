...
#pragma mark - 声明
@interface MXTIController()<UNUserNotificationCenterDelegate>

@property (nonatomic, strong) TITableView *table;
@property (nonatomic, strong) XXBottomButton *bottom;
...
- (void)timeListRequest {
    @weakify(self)
    [self createRequest:TimeListRequest params:nil complete:^(MXAPPResult *result) {
        @strongify(self)
...
- (void)addTimeRequest:(NSString *)time {
    ...
    [AFNNetworkManager POST:AddTimeRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
...
- (XXBottomButton *)bottom {
    if (!_bottom) {
        _bottom = [XXBottomButton initWithFrame:({
