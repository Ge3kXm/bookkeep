...
#import "XMBadgeController.h"
#import "CAController.h"
#import "XMWebVC.h"
#import "MXTIController.h"
...
- (void)getInfoRequest {
    ...
    [AFNNetworkManager POST:InfoRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
...
- (void)createPunchRequest {
    ...
    [AFNNetworkManager POST:CreatePunchRequest params:nil complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
        if (result.status == ServiceCodeSuccess) {
            UserModel *model = [UserInfo loadUserInfo];
...
- (void)soundChangeRequest:(NSNumber *)isOn {
    ...
    [AFNNetworkManager POST:SoundRequest params:param complete:^(MXAPPResult *result) {
        UserModel *model = [UserInfo loadUserInfo];
...
- (void)detailChangeRequest:(NSNumber *)isOn {
    ...
    [AFNNetworkManager POST:DetailRequest params:param complete:^(MXAPPResult *result) {
        UserModel *model = [UserInfo loadUserInfo];
...
//        else if (indexPath.row == 1) {
            MXTIController *vc = [[MXTIController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
//        }
    }
