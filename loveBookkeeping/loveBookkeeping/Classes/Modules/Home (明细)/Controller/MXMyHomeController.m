...
#pragma mark - 实现
@implementation MXMyHomeController
...
- (void)homeTablePull:(id)data {
    ...
    [self setModels:[BKMonthModel statisticalMonthWithYear:_date.year month:_date.month]];
    if([BKMonthModel statisticalMonthWithYear:_date.year month:_date.month].count < 1) {
        [_header setNotLaugh];
    } else {
        [_header setLaugh];
    }
...
- (HomeHeader *)header {
    if (!_header) {
        _header = [HomeHeader loadFirstNib:CGRectMake(0, _navigation.bottom, SCREEN_WIDTH, countcoordinatesX(164))];
