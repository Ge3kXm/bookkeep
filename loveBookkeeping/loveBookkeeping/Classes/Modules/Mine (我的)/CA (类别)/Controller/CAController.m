...
#import "XXBottomButton.h"
#import "XMACAController.h"
...
@property (nonatomic, strong) XXBottomButton *bootom;
...
- (void)getCategoryListRequest {
    @weakify(self)
    [self.table createRequest:CategorySetListRequest params:@{} complete:^(MXAPPResult *result) {
        @strongify(self)
...
- (void)removeSysCateRequest:(CategoryCell *)cell {
    ...
    [AFNNetworkManager POST:RemoveSystemCategoryRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
...
- (void)removeCustrCateRequest:(CategoryCell *)cell {
    ...
    NSDictionary *param = [NSDictionary dictionaryWithObjectsAndKeys:
                           @(1), @"customer_id", nil];
    [self showProgressHUD];
    [AFNNetworkManager POST:RemoveInsertCategoryListRequest params:param complete:^(MXAPPResult *result) {
        @strongify(self)
        [self hideHUD];
        if (result.status == ServiceCodeSuccess) {
...
- (void)categoryBtnClick:(id)data {
    XMACAController *vc = [[XMACAController alloc] init];
...
- (XXBottomButton *)bootom {
    if (!_bootom) {
        _bootom = [XXBottomButton initWithFrame:({
