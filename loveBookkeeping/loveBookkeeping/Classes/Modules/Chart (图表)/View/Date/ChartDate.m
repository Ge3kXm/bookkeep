...
//                    NSDate *date = [NSDate createZeroDate:[NSDate date]];
                    XMYChartSubModel *model = [[XMYChartSubModel alloc] init];
    ...
    [self.sModels replaceObjectAtIndex:0 withObject:({
        ...
        NSMutableArray<XMYChartSubModel *> *submodels = [[NSMutableArray alloc] init];
        ...
        for (NSInteger i=0; i<weeks; i++) {
            ...
            XMYChartSubModel *submodel = [XMYChartSubModel init];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 回调
    if (self.complete) {
        XMYChartSubModel *model = self.sModels[self.segmentIndex][indexPath.row];
        self.complete(model);
    }
}
...
- (NSMutableArray<NSMutableArray<XMYChartSubModel *> *> *)sModels {
    if (!_sModels) {
